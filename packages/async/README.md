...
```typescript
import { retry, createRetry, withRetry, TimeoutError } from '@tools/async';
...
- `@tools/utils` (tagged with `scope:shared`)

Attempting to import from `@tools/colors` or `@tools/strings` will result in a linting error due to module boundary constraints.
